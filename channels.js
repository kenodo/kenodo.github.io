export const channels = [
  {
    name: "ФРОСТ (выживание)",
    channelId: "UCb5nQ8aO7kU2pJ5l9D2vXWg"
  },
  {
    name: "ЧД",
    channelId: "UCzdhDZnwLa5_-xUletu1DhQ"
  },
  {
    name: "Чтиво",
    channelId: "UCeKCxQDv6lWDSzuqUXGtMRA"
  },
  {
    name: "Файб",
    channelId: "UUCGjqnAIqvrWts_pm4hLe5Eg"
  },
  {
    name: "Репич",
    channelId: "UC4xkz9m8Q0y7yM9zXvQYx6A"
  },
  {
  name: "Космос просто",
  channelId: "UCp4bMZ4cQ3A6z1zZ1p6WZ9A"
},

  // Космос и смежные
  { name: "Alpha Centauri", channelId: "UCi5iY9dF3vXqYQJzM8Z0W9A" },
  { name: "Kurzgesagt", channelId: "UCsXVk37bltHxD1rDPwtNM8Q" },
  { name: "PBS Space Time", channelId: "UC7_gcs09iThXybpVgjHZ_7g" },
  { name: "Everyday Astronaut", channelId: "UC6uKrU_WqJ1R2HMTY3LIx5Q" },

  // добьёшь сам — структура очевидна
];
